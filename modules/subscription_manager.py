# modules/subscription_manager.py - Refactored
...
class SubscriptionManager:
    """Manage company subscriptions and permissions - Supabase compatible"""
    
    def __init__(self, db):
        self.db = db
    
    # ──────────────────────────────────────────────────────────────
    # SUBSCRIPTION FETCHING - DELEGATES TO CRUD
    # ──────────────────────────────────────────────────────────────
    ...
    def get_plan_config(self, plan_name: str) -> Dict[str, Any]:
        ...
        return self.db.get_plan_by_name(plan_name) or {}
    ...
    def check_extension_limit(self, company_id: int) -> Tuple[bool, int, str]:
        ...
        try:
            ...
            plan_config = self.get_plan_config(plan_name)
            limit = plan_config.get('extension_auto_fills', 5)
            
            # Get current month usage
            used = self._get_extension_usage(company_id)
            ...
            logger.error(f"Error checking extension limit: {e}")
            return True, -1, "Unable to check limit, proceeding anyway"
    
    def _get_extension_usage(self, company_id: int) -> int:
        """Get current month's extension usage"""
        try:
            # Try Supabase first
            if hasattr(self.db, 'supabase') and self.db.supabase:
                ...
                response = self.db.supabase.table('extension_auto_fill_log') \
                    .select('count', count='exact') \
                    .eq('company_id', company_id) \
                    .gte('filled_at', start_of_month) \
                    .execute()
            ...
            logger.error(f"Error getting extension usage: {e}")
            return 0
    ...
    def check_limit(self, company_id: int, resource_type: str) -> Tuple[bool, int, str]:
        ...
        resource_map = {
            ...
            'analysis': ('analyses_limit', 'analyses_used', 'tender analyses'),
        ...
        # Get max limit from subscription or plan config
        max_limit = sub.get(max_field)
        if max_limit is None:
            plan_name = sub.get('plan', 'free')
            plan_config = self.get_plan_config(plan_name)
            max_limit = plan_config.get(max_field, 5)
        ...
        # Handle unlimited
        if max_limit == -1 or max_limit == 999999:
    ...
    def increment_usage(self, company_id: int, resource_type: str) -> bool:
        ...
        try:
            ...
            if hasattr(self.db, 'supabase') and self.db.supabase:
                ...
                current_value = sub.get(field, 0)
                
                self.db.supabase.table('subscriptions') \
                    .update({field: current_value + 1, 'updated_at': datetime.now().isoformat()}) \
                    .eq('id', sub['id']) \
                    .execute()
                return True
        ...
        try:
            if hasattr(self.db, 'supabase') and self.db.supabase:
                response = self.db.supabase.table('users') \
                    ...
                    .eq('is_active', True) \
                    .execute()
                return response.count if hasattr(response, 'count') else 1
            else:
                with self.db.get_connection() as conn:
                    cursor = self.db.get_cursor(conn)
                    cursor.execute("""
        ...
        try:
            ...
            if limit == -1 or limit == 999999:
                ...
                <div style="font-size: 0.8rem; color: #666; text-align: center; margin-top: 0.5rem;">
                    ...
                    <span style="color: #10b981;">♾️ Unlimited analyses</span>
                </div>
            ...
            logger.error(f"Error rendering usage stats: {e}")
    ...
    def render_extension_usage_stats(self, company_id: int = None) -> None:
        ...
        try:
            can_proceed, remaining, message = self.check_extension_limit(company_id)
            
            if remaining == -1:
                st.markdown("""
                <div style="font-size: 0.8rem; color: #666; text-align: center;">
                    <strong>🤖 Extension</strong><br>
                    <span style="color: #10b981;">♾️ Unlimited auto-fills</span>
                </div>
                """, unsafe_allow_html=True)
            else:
                # Get used count
                used = self._get_extension_usage(company_id)
                limit = 999999  # This should come from plan config
                
                # Get plan config for limit
                sub = self.get_company_subscription(company_id)
                plan_name = sub.get('plan', 'free')
                plan_config = self.get_plan_config(plan_name)
                limit = plan_config.get('extension_auto_fills', 5)
            ...
            logger.error(f"Error rendering extension usage: {e}")
    ...
    def render_subscription_badge(self, user_id: int = None) -> None:
        ...
        try:
            ...
            logger.error(f"Error rendering subscription badge: {e}")
...
def check_subscription_and_permission(db, resource_type=None, permission=None):
    ...
    if resource_type:
        can_proceed, _, message = sub_manager.check_limit(company_id, resource_type)
        if not can_proceed:
...
def check_subscription_access(company_id: int = None, user_id: int = None, subscription_manager: Optional['SubscriptionManager'] = None) -> Tuple[bool, str, str]:
    ...
    """
    logger.info("=" * 60)
    logger.info("🔍 check_subscription_access() called")
    logger.info("=" * 60)
    ...
    if user_role in ['system_admin', 'admin']:
        logger.info("   ✅ System admin - Full access granted")
    ...
    if subscription_manager is None:
        subscription_manager = SubscriptionManager(st.session_state.get('db'))
    
    sub = subscription_manager.get_user_subscription(user_id)
    plan = sub.get('plan', 'free')
    
    logger.info(f"   📊 Effective Plan: {plan}")
    
    if plan != 'free':
        logger.info(f"   ✅ Access granted - {plan.upper()} plan")
        return True, plan, f"Active subscription - {plan.upper()} plan"
    
    logger.info(f"   ❌ No active subscription found")
    return False, 'free', "No active subscription found. Please subscribe."


def check_premium_access(company_id: int, db) -> bool:
    """
