...
class RateCRUDForms:
    ...
    def _get_chapters_safe(self, source: str):
        """Safe method to get chapters"""
        table_name = "pwd_chapters" if source == "PWD" else "lged_chapters"
        try:
            conn = self.db.get_connection()
            cursor = conn.cursor()
            cursor.execute(f"""
            ...
            # Remove header contamination
            df = df[~df['chapter_number'].str.contains('chapter_number', case=False)]
            
            return df
        except Exception as e:
            st.error(f"Error: {e}")
            return pd.DataFrame(columns=['chapter_number', 'chapter_name', 'description'])
    ...
    def _chapter_crud(self, source, show_debug):
        ...
        if source == "PWD":
            chapters_df = self._get_chapters_safe(source)
        else:
            chapters_df = self._get_chapters_safe(source)
    ...
    def _section_crud(self, source, show_debug=False):
        ...
        chapters_df = self._get_chapters_safe("LGED")
    ...
    def _parent_crud(self, source, edition_year, show_debug):
        ...
        if existing_parents:
            ...
            if can_edit:
                ...
                if source == "PWD":
                    chapters_df = self._get_chapters_safe("PWD")
