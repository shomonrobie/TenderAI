...
class SupabaseContextWrapper:
    """Wrapper to make Supabase client work with context manager"""
    ...
    def cursor(self):
        return SupabaseCursorWrapper(self.client)
...
class SupabaseCursorWrapper:
    """Mimics SQLite cursor for Supabase queries"""
    def __init__(self, client):
        self.client = client
        self._result = []
        self._rowcount = 0
    ...
    def execute(self, sql: str, params: tuple = None):
        """Execute SQL statement - tries to parse and convert to Supabase API"""
        # Simple parsing for basic SELECT queries
        ...
        if sql_lower.startswith('select'):
            ...
            # For other queries, just return self
            self._result = []
            self._rowcount = 0
            return self
    ...
    @property
    def lastrowid(self):
        # Supabase doesn't have lastrowid in the same way
        return None
...
class DatabaseCRUD:
    ...
    def __init__(self, db_path="data/tender_system.db"):
        ...
        self.db_conn = db_connection  # Use the connection manager
        self.supabase = None  # <-- Initialize as None
        
        # Determine if Supabase is enabled
        self._use_supabase = os.getenv("USE_SUPABASE", "False").lower() == "true"
        ...
        if self._use_supabase:
            self._init_supabase()  # <-- ADD THIS LINE
        else:
            print(f"✅ DatabaseCRUD: SQLite mode enabled at {db_path}")
    ...
    def _init_supabase(self):
        ...
        try:
            ...
            if supabase_url and supabase_key:
                ...
                print("✅ Supabase client initialized in DatabaseCRUD")
            else:
    ...
    def get_db_type(self) -> str:
        ...
        return 'supabase' if self._use_supabase else 'sqlite'
    
    def get_connection(self):
        """
        Returns database connection (works with BOTH patterns)
        ...
        if self._use_supabase and hasattr(self, 'supabase') and self.supabase:
            # Return wrapper that supports context manager
            return SupabaseContextWrapper(self.supabase)
        else:
            # SQLite connection
    ...
    def query(self, sql: str, params: tuple = None) -> List[Dict]:
        ...
        if self._use_supabase:
            # Supabase returns dicts already
            return [dict(row) for row in rows]
        else:
            # SQLite - need manual conversion
            columns = [description[0] for description in cursor.description]
    ...
    def create_google_user(self, user_data: Dict) -> tuple:
        ...
        try:
            ...
            if google_id:
                existing_user = self.query_one(
                    "SELECT id, auth_provider, auth_provider_user_id FROM users WHERE auth_provider_user_id = ? AND auth_provider = 'google'",
                    (google_id,)
                )
            ...
            if not existing_user:
                existing_user = self.query_one(
                    "SELECT id, auth_provider, auth_provider_user_id FROM users WHERE email = ?",
                    (user_data['email'],)
                )
            
            if existing_user:
                # FIX: Access dict by key, not by index
                user_id = existing_user.get('id')
                auth_provider = existing_user.get('auth_provider')
                
                # If user exists, update with Google info if it's not already Google
                if google_id and auth_provider != 'google':
                    update_sql = """
                        ...
                        WHERE id = ?
                    """
                    self.execute(update_sql, (google_id, google_id, user_data['email'], user_data.get('picture', ''), user_id))
                    
                    # Also update user_oauth table if it exists
                    try:
                        oauth_sql = """
                            INSERT OR REPLACE INTO user_oauth (user_id, provider_name, provider_user_id, provider_email)
                            VALUES (?, 'google', ?, ?)
                        """
                        self.execute(oauth_sql, (user_id, google_id, user_data['email']))
                    except Exception as e:
                        print(f"⚠️ Could not update user_oauth: {e}")
            ...
            if mobile:
                ...
                if self.validate_bangladesh_mobile(mobile):
                    existing_mobile = self.query_one(
                        "SELECT id FROM users WHERE mobile_number = ?",
                        (mobile,)
                    )
                    if existing_mobile:
                        # Mobile exists, skip it
                        mobile = ''
            ...
            user_id = user.get('id')  # FIX: Use .get() instead of index
            
            # Also insert into user_oauth table if it exists
            if google_id:
                try:
                    oauth_sql = """
                    ...
                    print(f"⚠️ Could not insert into user_oauth: {e}")
                    # Non-critical error, continue
                    pass
            ...
            print(f"❌ Error in create_google_user: {e}")
            import traceback
            traceback.print_exc()
            return False, str(e)
            ...
            with self.get_connection() as conn:
                ...
                for row in rows:
                    ...
                    if isinstance(row_dict, dict):
                        user_ids.append(row_dict.get('id'))
                    elif isinstance(row_dict, (list, tuple)):
                        user_ids.append(row_dict[0] if len(row_dict) > 0 else None)
                ...
                if row:
                    row_dict = self._normalize_row(row)
                    if isinstance(row_dict, dict):
                        stats['total_analyses'] = row_dict.get('total', 0) or 0
                        stats['avg_confidence'] = row_dict.get('avg_conf', 0) or 0
                        stats['total_bids'] = row_dict.get('total_bids', 0) or 0
                        stats['total_wins'] = row_dict.get('wins', 0) or 0
                    elif isinstance(row_dict, (list, tuple)):
                        stats['total_analyses'] = row_dict[0] if len(row_dict) > 0 else 0
                        stats['avg_confidence'] = row_dict[1] if len(row_dict) > 1 else 0
                        stats['total_wins'] = row_dict[2] if len(row_dict) > 2 else 0
                        stats['total_bids'] = row_dict[3] if len(row_dict) > 3 else 0
    ...
    def get_company_stats_bak(self, company_id: int) -> Dict:
        """Get analysis statistics for a company"""
        with self.get_connection() as conn:
            cursor = self.db_conn.get_cursor(conn)
    ...
    def get_user_by_auth_provider_id(self, provider_user_id, provider='google'):
        """Get user by auth provider user ID"""
        return self.query_one(
            "SELECT * FROM users WHERE auth_provider_user_id = ? AND auth_provider = ?",
